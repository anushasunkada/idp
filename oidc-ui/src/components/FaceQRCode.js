...
export default function FaceQRCode({
  walletDetail,
  authService,
  openIDConnectService,
  backButtonDiv,
  secondaryHeading,
  i18nKeyPrefix1 = "FaceQRCode",
  i18nKeyPrefix2 = "errors",
}) {
  const post_GenerateToken = authService.post_GenerateBioToken;
  const post_TokenStatus = authService.post_BioStatus;
  ...
  */
  ...
  const fetchQRCode = async () => {
    ...
    try {
      ...
      let { response, errors } = await post_GenerateToken(
        openIDConnectService.getTransactionId()
      );

      if (errors != null && errors.length > 0) {
        let errorCodeCondition =
          langConfig.errors.wallet[errors[0].errorCode] !== undefined &&
          langConfig.errors.wallet[errors[0].errorCode] !== null;
  ...
  const triggerLinkStatus = async (
    ...
    try {
      ...
      while (timeLeft > 0) {
        try {
          linkStatusResponse = await post_TokenStatus(transactionId, token);
        } catch {
          //ignore
        }
      ...
      if (
        ...
        let errorCodeCondition =
          langConfig.errors.password[linkStatusResponse.errors[0].errorCode] !==
            undefined &&
          langConfig.errors.password[linkStatusResponse.errors[0].errorCode] !==
            null;
        ...
        if (errorCodeCondition) {
          ...
          linkStatusResponse.errors[0].errorCode === "invalid_transaction"
        ) {
          redirectOnError(
            linkStatusResponse.errors[0].errorCode,
      ...
      setErrorBanner({
        errorCode: "biometrics.auth_failed",
        show: true,
      });
    }
  };
        ...
        {qr && (
          <>
            ...
            <div className="w-full flex justify-center">
              <button
                className="border border-4 qrcode-border rounded-3xl p-2 hover:cursor-pointer"
